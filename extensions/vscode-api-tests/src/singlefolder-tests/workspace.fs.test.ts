...
suite('vscode API - workspace-fs', () => {
	...
	test('fs.write/stat/read/delete', async function () {
		...
		await vscode.workspace.fs.writeFile(uri, Buffer.from('HELLO') as Uint8Array);
	...
	test('fs.delete folder', async function () {
		...
		await vscode.workspace.fs.writeFile(file, Buffer.from('FOO') as Uint8Array);
	...
	test('fs.createFolder creates recursively', async function () {
		...
		await vscode.workspace.fs.writeFile(file, Buffer.from('Hello World') as Uint8Array);
