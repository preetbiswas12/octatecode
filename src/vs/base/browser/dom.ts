...
export function triggerDownload(dataOrUri: Uint8Array | URI, name: string): void {
	...
	if (URI.isUri(dataOrUri)) {
		...
		const ab = dataOrUri.buffer instanceof SharedArrayBuffer
			? new ArrayBuffer(dataOrUri.byteLength)
			: dataOrUri.buffer.slice(dataOrUri.byteOffset, dataOrUri.byteOffset + dataOrUri.byteLength);
