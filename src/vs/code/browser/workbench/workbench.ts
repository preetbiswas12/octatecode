...
class ServerKeyedAESCrypto implements ISecretStorageCrypto {
	...
	async unseal(data: string): Promise<string> {
		...
		const keyBuf = clientKey instanceof Uint8Array ? clientKey : (clientKey as unknown as Uint8Array);
		const key = await this.getKey(keyBuf as any);
		const ivBuf = iv instanceof Uint8Array ? (iv as Uint8Array<ArrayBuffer>) : (iv as unknown as Uint8Array<ArrayBuffer>);
		const cipherBufData = cipherText instanceof Uint8Array ? (cipherText as Uint8Array<ArrayBuffer>) : (cipherText as unknown as Uint8Array<ArrayBuffer>);
		const decrypted = await mainWindow.crypto.subtle.decrypt(
			{ name: AESConstants.ALGORITHM as const, iv: ivBuf },
			key,
			cipherBufData
		);
