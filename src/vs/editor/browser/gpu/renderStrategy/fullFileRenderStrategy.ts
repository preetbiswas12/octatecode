...
export class FullFileRenderStrategy extends BaseRenderStrategy {
	...
	public override onScrollChanged(e?: ViewScrollChangedEvent): boolean {
		...
		const ab = this._scrollOffsetValueBuffer.buffer instanceof SharedArrayBuffer
			? new ArrayBuffer(this._scrollOffsetValueBuffer.byteLength)
			: this._scrollOffsetValueBuffer.buffer.slice(this._scrollOffsetValueBuffer.byteOffset, this._scrollOffsetValueBuffer.byteOffset + this._scrollOffsetValueBuffer.byteLength);
		this._device.queue.writeBuffer(this._scrollOffsetBindBuffer, 0, ab);
		return true;
	}
