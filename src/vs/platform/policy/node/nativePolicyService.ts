...
export class NativePolicyService extends AbstractPolicyService implements IPolicyService {
	...
	protected async _updatePolicyDefinitions(policyDefinitions: IStringDictionary<PolicyDefinition>): Promise<void> {
		...
		let createWatcher: any;
		try {
			const module = await import('@vscode/policy-watcher');
			createWatcher = module.createWatcher;
		} catch (err) {
			this.logService.warn(`NativePolicyService#_updatePolicyDefinitions - Failed to load native policy watcher:`, err);
			return; // Gracefully skip if native module is not available
		}
