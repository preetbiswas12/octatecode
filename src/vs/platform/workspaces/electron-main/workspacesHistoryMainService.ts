...
export class WorkspacesHistoryMainService extends Disposable implements IWorkspacesHistoryMainService {
	...
	private async updateWindowsJumpList(): Promise<void> {
		...
		try {
			...
			if (res && res !== 'ok') {
				// Suppress privacy-related errors (Windows 10+ privacy settings)
				if (res !== 'customCategoryAccessDeniedError') {
					this.logService.warn(`updateWindowsJumpList#setJumpList unexpected result: ${res}`);
				}
			}
		} catch (error) {
			// Silently ignore privacy-related errors from Windows
			const errorMsg = error instanceof Error ? error.message : String(error);
			if (!errorMsg.includes('privacy') && !errorMsg.includes('access denied')) {
