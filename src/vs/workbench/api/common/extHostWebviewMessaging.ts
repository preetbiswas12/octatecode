	...
	if (options.serializeBuffersForPostMessage) {
		...
		const replacer = (_key: string, value: any) => {
			if (value instanceof ArrayBuffer) {
				...
				if (type) {
					const buf = value.buffer instanceof ArrayBuffer ? value.buffer : value.buffer instanceof SharedArrayBuffer ? new ArrayBuffer(value.byteLength) : value.buffer;
					const index = arrayBuffers.add(buf);
					return {
						$$vscode_array_buffer_reference$$: true,
						index,
						view: {
