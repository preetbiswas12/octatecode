...
export class ImageAttachmentWidget extends AbstractChatAttachmentWidget {
	...
	private createImageElements(buffer: ArrayBuffer | Uint8Array, widget: HTMLElement, hoverElement: HTMLElement) {
		const bufferSource = buffer instanceof Uint8Array ? buffer : new Uint8Array(buffer);
		let bufferToBlob: BlobPart;
		if (bufferSource instanceof Uint8Array && bufferSource.buffer instanceof ArrayBuffer) {
			bufferToBlob = bufferSource as Uint8Array<ArrayBuffer>;
		} else if (bufferSource instanceof Uint8Array) {
			bufferToBlob = bufferSource.slice(0) as Uint8Array<ArrayBuffer>;
		} else {
			bufferToBlob = bufferSource;
		}
		const blob = new Blob([bufferToBlob], { type: 'image/png' });
