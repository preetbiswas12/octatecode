	...
	private async _unsealSecrets(): Promise<Record<string, IResolvedValue>> {
		...
		try {
			...
			const decrypted = await crypto.subtle.decrypt(
				{ name: MCP_ENCRYPTION_KEY_ALGORITHM, iv: iv.buffer instanceof ArrayBuffer ? iv.buffer : (iv as any).buffer },
				key,
				encrypted.buffer instanceof ArrayBuffer ? encrypted.buffer : (encrypted as any).buffer,
