	...
	function createOutputItem(
		...
		function create(
			...
			return Object.freeze<ExtendedOutputItem>({
				...
				blob(): Blob {
					const bufferData = valueBytes instanceof Uint8Array && valueBytes.buffer instanceof ArrayBuffer ? valueBytes : new Uint8Array((valueBytes as unknown as ArrayBuffer));
					return new Blob([(bufferData as unknown as ArrayBuffer)], { type: this.mime });
				}, get _allOutputItems() {
		...
		constructor(id: string, mime: string, content: string, top: number, metadata: NotebookCellMetadata) {
			...
			this.outputItem = Object.freeze<ExtendedOutputItem>({
				...
				blob(): Blob {
					...
					const bufferData = data instanceof Uint8Array ? data : new Uint8Array((data as unknown as ArrayBuffer));
					return new Blob([(bufferData as unknown as ArrayBuffer)], { type: this.mime });
				}, _allOutputItems: [{
					mime,
