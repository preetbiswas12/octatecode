	...
	async call(_: unknown, command: string, params: any): Promise<any> {
		try {
			if (command === 'refreshMCPServers') {
				...
				throw new Error(`OctateCode sendLLM: command "${command}" not recognized.`)
			}
		}
		catch (e) {
	...
	private async _createClientUnsafe(server: MCPConfigFileEntryJSON, serverName: string, isOn: boolean): Promise<ClientInfo> {
		...
		if (server.url) {
			...
			try {
				...
				const toolsWithUniqueName = tools.map(({ name: toolName, ...rest }: any) => ({ name: this._addUniquePrefix(toolName), ...rest }))
				info = {
