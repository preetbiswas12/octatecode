...
export class NativeLocalProcessExtensionHost implements IExtensionHost {
	...
	private async _start(): Promise<IMessagePassingProtocol> {
		...
		if (!this._environmentService.isBuilt && !this._environmentService.remoteAuthority || this._isExtensionDevHost) {
			// Increase timeout to 30 seconds for development mode with collaboration services
			// Use environment service instead of process.env (available in browser context)
			const STARTUP_TIMEOUT = !this._environmentService.isBuilt ? 30000 : 10000;
			startupTimeoutHandle = setTimeout(() => {
				this._logService.error(`[LocalProcessExtensionHost]: Extension host did not start in ${STARTUP_TIMEOUT / 1000} seconds (debugBrk: ${this._isExtensionDevDebugBrk})`);
