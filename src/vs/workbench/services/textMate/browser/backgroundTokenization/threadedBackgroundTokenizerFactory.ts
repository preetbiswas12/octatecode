	...
	private async _createWorkerProxy(): Promise<Proxied<TextMateTokenizationWorker> | null> {
		...
		TextMateWorkerHost.setChannel(worker, {
			...
			$setTokensAndStates: async (controllerId: number, versionId: number, tokens: Uint8Array, lineEndStateDeltas: StateDeltas[]): Promise<void> => {
				...
				if (controller) {
					const tokensView = tokens instanceof Uint8Array ? tokens : (tokens as unknown as Uint8Array);
					const statesView = lineEndStateDeltas instanceof Uint8Array ? lineEndStateDeltas : (lineEndStateDeltas as unknown as Uint8Array);
					controller.setTokensAndStates(controllerId, versionId, ((tokensView.buffer as unknown as ArrayBuffer)), (statesView as any));
				}
			},
		...
		await worker.proxy.$init(createData); if (this._worker !== worker) {
